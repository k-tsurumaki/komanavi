...
私自身、引っ越し手続きを調べていて「もう窓口で聞いたほうが早い」と画面を閉じた経験があります。
普段からWebに触れていて、調べる時間もある自分ですらそうなのだから——**子育て世帯、高齢者、外国人住民、災害被災者にとって、このハードルはどれほど高いだろうか。**
...
行政ページを「読んで終わり」にしないために、KOMANAVIでは **URL入力から行動計画の完成まで** を7つのステップに分解しました。画面上の進捗インジケーターが「今どこにいて、次に何をすればいいか」を常に示します。
...
### 機能 ３：漫画で確認 —— 「他人事」を「自分ごと」に変える

前章で示した「なぜ漫画なのか」という仮説を、プロダクトとして具現化したKOMANAVIの中核機能です。

Gemini 3.0 Pro Image が、ここまでのステップで構築された中間表現——ページ概要・ユーザーの意図・プロフィール情報——を統合的に読み取り、**4〜8コマの漫画** を自動生成します。

#### 3つの設計判断

| 設計判断 | 内容 | 狙い |
|---------|------|------|
| **コマ構成をモデルに委ねる** | 何を・どの順で・何コマで描くかを、ページの性質に応じてGeminiが自律的に決定 | 制度ごとに異なる情報構造へ柔軟に対応 |
| **自分が主人公になる** | Myページで登録した外見・性格が漫画の主人公にそのまま反映される | 「誰かの話」ではなく「私の話」として届く |
| **非同期生成** | Cloud Tasks経由でWorkerにオフロードし、生成中も要約やチェックリストを閲覧可能 | 30秒〜2分の生成待ちを体感ゼロに近づける |

「児童手当の申請手順」という文字の羅列を読むのと、**自分の姿で窓口に向かう場面を漫画で見る** のとでは、情報の届き方がまるで違います。行政情報が「他人事」から **「自分ごと」** に変わる——Learning（学習コスト）と Psychological（心理コスト）を同時に引き下げる、KOMANAVIの核心です。

#### 実際に生成された漫画

以下は、ある行政ページのURLを入力し、KOMANAVIが自動生成した漫画の一例です。コマの数・構成・セリフのすべてをGemini 3.0 Pro Imageが自律的に決定しています。

![KOMANAVIが自動生成した漫画のサンプル](../images/manga_sample.png)
...
### Identity Platform × Cloud Run —— APIキー不要の認証基盤

ブラウザ側は **Firebase Auth SDK**（Google OAuth / メールPW）、サーバー側は **Firebase Admin SDK** でIDトークンを検証し、**Auth.js（NextAuth v5）** がセッションを管理します。バックエンドは GCP Identity Platform に統一されており、Cloud Run のサービスアカウント認証により APIキー管理は不要です。
...
### Gemini API × Google Search Grounding —— 「スクレイピングしない」という判断
...
### Cloud Tasks × Cloud Run —— 漫画生成の非同期パイプライン
...
### Cloud Build × Artifact Registry —— 並列ビルドで自動デプロイ
...
多言語対応も含め、現在のKOMANAVIはあくまで「作り手の仮説」に基づいて設計されています。**本当に「入口の詰まり」を解消できているかは、実際のユーザーに使ってもらわなければ検証できません。** そこで、以下の3段階でユーザーテストを計画しています。
...
なかでも特に重視しているのは、**「KOMANAVIを使った後に、実際に窓口へ行けたか・申請できたか」** という行動変容の測定です。「わかりやすかった」という主観的な感想にとどまらず、Complianceコストが実際に下がったかどうかまで追跡することで、プロダクトの本質的な価値を検証していきます。


## おわりに

開発中、何十もの行政ページと向き合うなかで痛感したのは、この「届かない」という問題の根深さでした。
