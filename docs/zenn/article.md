...
- **人手不要** — AIがリアルタイムにページを解析。制度改定への追随も自動
- **質問不要** — URLを貼るだけで始まる。プロンプトを考える必要がない
- **要約で終わらない** — 意図入力 → パーソナライズ回答 → チェックリスト → 漫画まで、**「行動」に繋げる導線** を一気通貫で提供
...
生成される回答は4ブロック構造に圧縮されます：

| ブロック | 内容 | 狙い |
|---------|------|------|
| **結論** | 回答の要点を一文で | 長文を読まずに全体像を即座に把握 |
| **あなたは対象になりそうですか？** | 対象可能性の判断 | 「自分が該当するのか」の最速回答 |
| **最優先の1手** | 最初にやるべきアクション | 「何から手をつけるか分からない」の解消 |
| **見落とすと申請で困るポイント** | 期限切れ・書類不備などのリスク | 行政手続き特有の落とし穴を事前警告 |

:::message
Myページにプロフィール（年齢・職業・居住地・国籍など）を登録しておくと、回答が自動でパーソナライズされます。例えば外国籍の方には在留カード関連の項目が追加されるなど、**属性に応じて提示する情報の優先順位が変わります**。
:::
