...
### 8.2 ページ理解モードの証跡 (`SummaryViewer`)

- `overview.evidenceByBlock` を最優先
- 欠損時は既知URLプールからブロック単位でフォールバック
- 入力元URL（`source_url`）だけしかない場合は証跡表示を抑制
- 各コンテンツブロック内には表示せず、ページ理解モード末尾の「証跡URL」ブロックに集約表示

### 8.3 意図回答の証跡 (`result/page.tsx`)
...
1. `/result` でページ理解モードの主要ブロックが表示される（`このページの最重要ポイント` と `問い合わせ情報` はデータがある場合のみ表示）
